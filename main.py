...
from evaluation import calculate_eer_for,calculate_tDCF_EER
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.fine_tuning or args.eval:
        dev_trial_path = (database_path / "protocols/dev.trl.txt")
        eval_trial_path = (database_path / "protocols/eval.trl.txt")
    else:
    ...
    model_tag = "{}_{}_ep{}_bs{}_{}".format(
        ...
        config["num_epochs"], config["batch_size"],  args.sr_model)
    if args.comment:
    ...
    trn_loader, dev_loader, eval_loader = get_loader(
        parser.parse_args(),database_path, args.seed, config)

    # evaluates pretrained model and exit script
    if args.eval or args.basic19_eval:
        ...
        print("DONE.")
        eval_eer,threshold, min_tDCF = calculate_tDCF_EER(
            cm_scores_file=eval_score_path,
            output_file=model_tag/"loaded_model_t-DCF_EER.txt",
            asv_score_file = config["asv_score_path"])
        print('eval_eer:',eval_eer,'threshold:',threshold,'min_tDCF:',min_tDCF)
        sys.exit(0)
    ...
    produce_evaluation_file(eval_loader, model, device, eval_score_path,
                            eval_trial_path)
    eval_eer = calculate_eer_for(cm_scores_file=eval_score_path,
...
def get_loader(
    ...
    if args.fine_tuning or args.eval:
        ...
        trn_list_path = (database_path / "protocols/train.trn.txt")
        dev_trial_path = (database_path / "protocols/dev.trl.txt")
        eval_trial_path = (database_path / "protocols/eval.trl.txt")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_score_name', type=str, help="eval score file name, end with .txt", default=None)
    
    parser.add_argument(
        "--eval",
        action="store_true",
        help="when this flag is given, evaluates given model with our dataset and exit")
    parser.add_argument(
        "--basic19_eval",
        action="store_true",
        help="when this flag is given, evaluates given model with LA19 dataset and exit")
    parser.add_argument(
        "--fine_tuning",
        action="store_true",
        help="when this flag is given, fine tuning given model")
