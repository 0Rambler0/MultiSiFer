...
class Model(nn.Module):
    def __init__(self, args, sr_model, device):
        ...
        if args["feature_layers"] != -1:
            layers_id = range(args["feature_layers"][0],args["feature_layers"][1]) 
            self.featurizer = Featurizer(self.upstream_model, layers_id)
        else: 
            self.featurizer = Featurizer(self.upstream_model)
    ...
    def forward(self, x, x_len):
        ...
        # print(hs.shape)
        # hs = hs[:,0:hs_len[0],0:hs_len[1]]
        x = self.LL(hs) #(bs,frame_number,feat_out_dim)
